...
# .equ    STACK_POINTER,     0xXXXX
  .equ    STACK_POINTER,     0x07e01  # Stack starts at this address
# .equ    STACK_SEGMENT,     0xXXXX
  .equ    STACK_SEGMENT,     0x9fbff  # Stack ends at this address
...
_start:
  jmp  over                     # Jumps to this mark
os_size:      
  .word 0
over:
  # setup stack
  movw  $STACK_SEGMENT, %ax     # Moving end-address of stack to ax register
  movw  %ax, %ss                # Moving end-address of stack to "Stack Segment" register
  movw  $STACK_POINTER, %sp     # Moving stack pointer to "Stack Pointer" register

  # setup data segment
  movw  $BOOT_SEGMENT, %ax      # Moving "start of boot block address" to ax register
  movw  %ax, %ds                # Moving "start of boot block address" to "Data segment" register
