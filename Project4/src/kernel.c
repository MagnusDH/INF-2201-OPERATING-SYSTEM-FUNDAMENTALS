...
{
	//Reading first block from storage device into OS_block buffer 
	unsigned char OS_block[SECTOR_SIZE];
	scsi_read(0, 1, (char*)OS_block);						//Block 0, 1 sector, read into OS_block buffer

	//Using OS_block buffer to find the size of Operating System
	int OS_size = (int)OS_block[2];

	//Process directory is located at sector 1 plus OS size 
	int proc_dir = OS_size + 1;

	//Overwriting OS_block with the process directory 
	scsi_read(proc_dir, 1, (char*)OS_block);

	//Copy the entire block from OS_block into given buffer	
	bcopy((char*)OS_block, (char*)buf, SECTOR_SIZE);
...
int loadproc(int location, int size) 
{
	...
	uint32_t space = alloc_memory((size * SECTOR_SIZE) + STACK_SIZE);
	
	//Read code/data from disk/directory
	scsi_read(location, size, (char*)space);

	//Create new PCB and place it in ready queue
	create_process(space, (size * SECTOR_SIZE) + STACK_SIZE);
