...
static unsigned int start_addr[NUM_THREADS] = {
    ...
    (unsigned int)usb_thread,    /* Scans USB hub port */
 ...
 * which is in entry.S.
 */
static handler_t exception_handler[NUM_EXCEPTIONS] = {exception_0, exception_1, exception_2, exception_3, exception_4, exception_5, exception_6, exception_7, exception_8, exception_9, exception_10, exception_11, exception_12, exception_13, exception_14_entry};

/* This is the entry point for the kernel */
FUNCTION_ALIAS(kernel_start, _start);
void _start(int load_size) {
	int i;

	/* Magic to set the otherwise constant os_size. (We must not change it later,
	 * as it is used by the file system.)*/
	*((int *)&os_size) = load_size;
	...
	init_syscall(SYSCALL_FS_MKFS, (syscall_t)fs_mkfs);
	init_syscall(SYSCALL_FS_OPEN, (syscall_t)fs_open);
	init_syscall(SYSCALL_FS_CLOSE, (syscall_t)fs_close);
	init_syscall(SYSCALL_FS_READ, (syscall_t)fs_read);
	init_syscall(SYSCALL_FS_WRITE, (syscall_t)fs_write);
	init_syscall(SYSCALL_FS_LSEEK, (syscall_t)fs_lseek);
	init_syscall(SYSCALL_FS_LINK, (syscall_t)fs_link);
	init_syscall(SYSCALL_FS_UNLINK, (syscall_t)fs_unlink);
	init_syscall(SYSCALL_FS_STAT, (syscall_t)fs_stat);
	init_syscall(SYSCALL_FS_MKDIR, (syscall_t)fs_mkdir);
	init_syscall(SYSCALL_FS_CHDIR, (syscall_t)fs_chdir);
	init_syscall(SYSCALL_FS_RMDIR, (syscall_t)fs_rmdir);
...
void create_segment(struct segment_t *entry, /* GDT entry */
                    ...
                    char type, char privilege /* privilege level */, char system /* system bit (0: system segm.) */) {
...
void init_gdt(void) {
	...
	create_segment(gdt + TSS_INDEX, (uint32_t)&tss, TSS_SIZE, TSS_SEGMENT, 0, SYSTEM);
...
void print_status(int time) {
	...
	i = 0;
	do {
		scrprintf(base + i, 0, "%-5d%-10s%-10s%-10d%-10d%-10d%-10d%-10x", p->pid, p->is_thread ? "Thread" : "Process", status[p->status], p->disable_count, p->preempt_count, p->yield_count, p->page_fault_count, p->kernel_stack);
		p = p->next;
		i++;
