...
static unsigned int start_addr[NUM_TOTAL] = {
	...
	(unsigned int) num,				//Philosophers
	(unsigned int) caps,			//Philosophers
	(unsigned int) scroll_th,		//Philosophers
	(unsigned int) mcpi_thread0,
	(unsigned int) mcpi_thread1,
	(unsigned int) mcpi_thread2,
	(unsigned int) mcpi_thread3,
	
	/*
	 * Set NUM_THREADS to 13 and uncomment these lines to test barrier
	 * implementation
	 */
	
	(unsigned int) barrier1,
	(unsigned int) barrier2,
	(unsigned int) barrier3,
	
	PROC1_ADDR,     /* given in Makefile */
	PROC2_ADDR
};
...
void _start(void) {
	...
	for (i = 0; i < NUM_TOTAL; i++) {
		pcb[i].pid = i;
		pcb[i].is_thread = (i < NUM_THREADS) ? TRUE : FALSE;
		pcb[i].nested_count = (i < NUM_THREADS) ? TRUE : FALSE;
		...
		/* if process then setup user stack */
		if (!pcb[i].is_thread) {
			ASSERT(next_stack < STACK_MAX);
...
void print_status(void) {
	...
	for (i = 0; i < NUM_TOTAL && (base + i) < 25; i++) {
