...
static void print_trace(int thread, int status) {
	print_str(LOCKTS_LINE - 1, 0, "Thread 2");
	print_str(LOCKTS_LINE - 1, 10, "Thread 3");
	print_str(LOCKTS_LINE - 1, 20, "Result");
	if (status == RUNNING) {
		if (thread == 2)
			print_int(LOCKTS_LINE, 0, shared_var);
		else
			print_int(LOCKTS_LINE, 10, shared_var);

		print_str(LOCKTS_LINE, 20, "Running");
	}
	else if (status == OK){
		print_int(LOCKTS_LINE, 10, shared_var);
		print_str(LOCKTS_LINE, 20, "Passed ");
	}

	else
		print_str(LOCKTS_LINE, 20, "Failed ");
