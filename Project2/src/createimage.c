...
static void write_segment(struct image_t *im, Elf32_Phdr phdr);
...
static void write_bootloader_signature(FILE *img);
...
static void create_images(int nfiles, char *files[]) {
	...
	if (options.vm == 1) {
		...
		write_bootloader_signature(image.img);
	}
	else {
...
static void write_process_directory(struct image_t *im) {
	...
	if (im->pd_loc + (int)sizeof(struct directory_t) >= im->pd_lim) {
...
static void write_bootloader_signature(FILE *img) {
	const int16_t signature = BOOT_LOADER_SIG_2 << 8 | BOOT_LOADER_SIG_1;

	if (options.extended == 1) {
		printf("writing bootable signature: 0x%04x\n", 0xffff & (int)signature);
	}

	int pos = ftell(img);
	fseek(img, BOOT_LOADER_SIG_OFFSET, SEEK_SET);
	fwrite(&signature, sizeof(int16_t), 1, img);
